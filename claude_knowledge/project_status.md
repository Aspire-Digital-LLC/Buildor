...
- [x] **Forced permission pipeline (all tools)**: Sessions start with `--settings` that clears Claude's allow list, forcing every tool call through the permission prompt. Buildor-managed auto-approve rules (`autoApprove.ts`, persisted in `~/.buildor/config.json`) replace `settings.local.json`. "Always Allow" writes to Buildor's store. All paths remain pool-gated for full telemetry visibility.
