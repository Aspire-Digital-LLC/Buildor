...
- [x] **Buildor-managed auto-approve**: All tool calls forced through permission prompt (Claude allow list cleared). Buildor-managed rule store in `config.json`, pattern matching (`"Read"`, `"Bash(git:*)"`, etc.), "Always Allow" writes to Buildor store. Pool-gated — auto-approved tools still scheduled through operation pool for full telemetry visibility.
- [x] **Skill Builder panel**: Three-panel visual editor (browser + 7-tab editor + restricted Sonnet chat). Covers all skill.json fields + prompt.md. Dirty tracking, save-with-commit to shared memory repo. Skill scope (general/project) with palette filtering.
- [x] **SDK Service Phase 1**: Standalone Node.js HTTP server wrapping Claude Agent SDK (`src-tauri/sdk-service/`). REST/SSE endpoints for session CRUD, streaming, messaging, permissions, model switching, health. NDJSON wire format matching CLI output. PreToolUse permission hooks. Smoke test suite. Solves CMD window flashing, stdout backpressure, and permission IPC overhead.

### In Progress
- [ ] **SDK Service Phase 2**: Rust integration — sidecar lifecycle management (spawn/health-check/restart), HTTP client in Rust replacing current `std::process::Command` spawning, SSE proxy to frontend. See `claude_knowledge/sdk_service_spec.md`.
